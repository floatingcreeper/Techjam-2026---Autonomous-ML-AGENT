...
class Budget:
    ...
    is authoritative. Restored to 3. See docs/SYSTEM.md §16.
...
@dataclass
class Research:
    """Internal research bookkeeping. MUST NOT postpone official convergence (docs/SYSTEM.md §16).
    ...
    """
    # statistical adoption (docs/SYSTEM.md §13) -- paired user-level bootstrap on saved predictions, no retraining
    ...
    # proposal hygiene (docs/SYSTEM.md §12)
    ...
    # plateau escalation (docs/SYSTEM.md §16) -- changes proposal policy, never the stopping rule
    plateau_after: int = 2             # consecutive uninformative experiments before escalating
    explore_p_escalated: float = 0.40
    # portfolio (docs/SYSTEM.md §15)
    cv_folds: int = 5
    max_members: int = 4
    weight_step: float = 0.25
    # integrity (docs/SYSTEM.md §8) -- a valid primary this high is a bug or a leak, never a model
...
class Config:
    ...
    # cross-run evidence ledger (docs/SYSTEM.md §14)
    ledger_path: str = "runs/_ledger.jsonl"
    use_ledger: bool = True
    # 3B -- Lever E unbiased-exposure eval, reported as a SECOND surface (docs/RESEARCH.md §15)
    unbiased_eval: bool = False
    # structured event stream for the Research Console (docs/SYSTEM.md §19)
    events: bool = True
