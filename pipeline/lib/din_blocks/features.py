...
alongside its video id -- docs/EN/RESEARCH.md §11. It is carried as an optional third element of the
`seq` tuple because the frozen `FeatureSet` cannot gain a field.

Leakage discipline: the feedback array is built by the chronological cache (docs/EN/RESEARCH.md §10), where a history
...
def build_features(bundle, cfg):
    ...
    # near-oracle label proxies held outside the block-visible cache (docs/EN/SYSTEM.md §8).
    aux = None
