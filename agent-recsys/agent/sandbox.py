...
def run_scratch(scratch_dir: Path, data_dir: str, timeout_s: int = TIMEOUT_S) -> RunResult:
    ...
    # Only 'valid' is required per-iteration -- see pipeline/_run_iteration.py's
    # HIDDEN-TEST COMPLIANCE note: 'test' is deliberately never computed
    # during iteration, so it must not be required here either. A stray
    # 'test' key is tolerated (not scrubbed) rather than rejected, since
    # rejecting it would just be a second, redundant enforcement point --
    # the real enforcement is upstream, in what data _run_iteration.py
    # hands to run_fm() in the first place.
