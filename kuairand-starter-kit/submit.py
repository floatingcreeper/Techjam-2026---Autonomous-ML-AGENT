...
if __name__ == '__main__':
    # Command-line entry point. Exactly one of --make / --check / --score is
    # required (the argparse mutually-exclusive group below enforces this):
    #   --make   trains an example FM and WRITES a submission CSV.
    #            python3 submit.py --make  --split test  submission.csv
    #   --check  only VALIDATES an existing CSV's format/alignment — no
    #            scoring, so it works even for `test` (where you have no
    #            true labels to score against anyway).
    #            python3 submit.py --check --split test  submission.csv
    #   --score  validates AND scores — only meaningful for `valid`, since
    #            that's the only split whose true labels you actually have.
    ...
    if a.make:
        ...
        # Same training-loop shape as baseline.run_fm: train one epoch,
        # check validation primary, keep the best checkpoint, stop early
        # after 4 epochs without improvement (patience=4). No per-epoch
        # printing here though — this path is meant to just produce a file,
        # not a progress log.
        for ep in range(40):
            ...
            if p > best + 1e-5: best, bad, state = p, 0, (m.V.copy(), m.W.copy(), m.b)
                ...
                bad += 1
                if bad >= 4: break
        m.V, m.W, m.b = state                    # roll back to best-validation checkpoint, same as baseline.py
        # Predict on whichever split was requested (valid or test) and write
        # it out in the official row_id,user_id,video_id,score format.
